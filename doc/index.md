...
├── UI 框架 (src/ui/)
│   └── [UI 核心框架](ui/index.md)
└── 可视化工具 (doc/visualizer/)
    └── [Xeros 内核流程可视化器](visualizer/index.html)
```
...
- **[Xeros 内核流程可视化器](visualizer/index.html)** — 交互式 SVG 动画演示：启动、上下文切换、调度、SMP/IPI、同步原语、VFS、中断、内存分配、寄存器配置、Shell 执行
...
| [Xeros 内核流程可视化器](visualizer/index.html) | 交互式流程演示 | 10 个内核模块动画、寄存器面板、源码跳转 |
