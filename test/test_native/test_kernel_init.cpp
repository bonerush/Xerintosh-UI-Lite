...
TEST(KernelPanicTest, GetLastPanicInitiallyNull)
{
    kern_clear_panic();
    /* 初始状态下不应该有 panic 记录 */
    EXPECT_EQ(kern_get_last_panic(), nullptr);
}

TEST(KernelPanicTest, PanicStoresMessage)
{
    kern_clear_panic();
    kern_panic("test panic message");

    const char *msg = kern_get_last_panic();
    ASSERT_NE(msg, nullptr);
    EXPECT_STREQ(msg, "test panic message");
}

TEST(KernelPanicTest, ClearPanicWorks)
{
    kern_panic("before clear");
    kern_clear_panic();
    EXPECT_EQ(kern_get_last_panic(), nullptr);
}

TEST(KernelPanicTest, MultiplePanicsUseLastMessage)
{
    kern_clear_panic();
    kern_panic("first panic");
    kern_panic("second panic");

    const char *msg = kern_get_last_panic();
    ASSERT_NE(msg, nullptr);
    EXPECT_STREQ(msg, "second panic");
}

/* ═══ 统计信息测试 ═══ */

TEST(KernelStatsTest, LogStatsDoesNotCrash)
{
    kern_init();
    /* 统计输出不应该崩溃 */
    kern_log_stats();
