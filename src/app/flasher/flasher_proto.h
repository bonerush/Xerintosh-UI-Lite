 ...
 * @details 从 USB→UART 数据流中自动识别 STK500 (avrdude) 和 ESP32 SLIP (esptool) 协议，
 *          并计算烧录进度百分比。所有解析器内部状态封装在 .c 中，本头文件仅暴露最小接口。
 *
 *          使用流程：
 ...
 * @param  byte  原始字节
 * @return 当前已确认的协议类型
 * @note   未确认时双解析器同时运行；任一解析器先锁定后锁定协议类型。
 ...
 * @note   清空协议类型、STK500 状态机、SLIP 缓冲区、进度计数器
 */
