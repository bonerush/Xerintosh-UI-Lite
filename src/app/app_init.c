...
#define FLASHER_ROLE_OPTION_COUNT 3

/* 子菜单项内容缓冲区：3 个端口号 x 3 个角色选项 */
static char g_sub_label_bufs[3][3][24];
            ...
            if (dur >= 800) {
                ...
                if (g_flasher_pin_menu != NULL) {
                    if (owner_idx < g_flasher_pin_menu->child_num)
                        safe_set_content(g_flasher_pin_menu->child_list_item[owner_idx], g_pin_label_bufs[owner_idx]);
                ...
                snprintf(hint, sizeof(hint), "已被 G%d 占用\n长按 %.1fs 解除",
                         g_flasher_sub_owner_pin, (800.0f - dur) / 1000.0f);
                xerintosh_push_pop_up(hint, 100);
            }
        } else {
            /* BtnA 已松开，取消 */
            g_flasher_sub_state = FLASHER_SUB_IDLE;
