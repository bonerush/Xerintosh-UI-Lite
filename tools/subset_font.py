...
def main():
    ...
    if not os.path.exists(tmpfile):
        # Search for the efont_cn object file (library index varies per build)
        objfiles = glob.glob(".pio/build/m5stick-c/*/LovyanGFX/lgfx/Fonts/efont/lgfx_efont_cn.o")
        if not objfiles:
            # Fallback: try M5GFX (older structure)
            objfiles = glob.glob(".pio/build/m5stick-c/*/M5GFX/lgfx/Fonts/efont/lgfx_efont_cn.c.o")
        if not objfiles:
            print("Error: lgfx_efont_cn.o not found. Build first with 'pio run -e m5stick-c'")
            sys.exit(1)
        objfile = objfiles[0]
